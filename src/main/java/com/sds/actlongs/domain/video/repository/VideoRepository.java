...
public interface VideoRepository extends JpaRepository<Video, Long> {
	...
	@Query("SELECT v "
		+ "FROM videos v "
		+ "INNER JOIN v.board b ON b.member.id = :memberId AND b.channel.id = :channelId AND b.status IN ('UPLOADING','COMPLETED') "
		+ "ORDER BY v.createdAt DESC")
	List<Video> findByBoardMemberIdOrderByCreatedAtDesc(Long memberId, Long channelId);

	@Query("SELECT v "
		+ "FROM videos v "
		+ "INNER JOIN boards b ON v.board.id = b.id AND b.status IN ('UPLOADING','COMPLETED') "
