...
public interface VideoRepository extends JpaRepository<Video, Long> {
	...
	@Query("SELECT v "
		+ "FROM videos v "
		+ "INNER JOIN v.board b ON b.status <> 'DELETED' "
		+ "WHERE v.board.id = :boardId")
	Optional<Video> findByBoardId(Long boardId);

	@Query("SELECT v "
		+ "FROM videos v "
		+ "INNER JOIN v.board b ON b.channel.id = :channelId AND b.status <> 'DELETED' "
		+ "ORDER BY v.createdAt DESC")
	List<Video> findByBoardChannelIdOrderByCreatedAtDesc(Long channelId);

	@Query("SELECT v "
		+ "FROM videos v "
		+ "INNER JOIN v.board b ON b.member.id = :memberId AND b.status <> 'DELETED' "
		+ "ORDER BY v.createdAt DESC")
	List<Video> findByBoardMemberIdOrderByCreatedAtDesc(Long memberId);

	@Query("SELECT v "
		+ "FROM videos v "
		+ "INNER JOIN boards b ON v.board.id = b.id AND b.status <> 'DELETED' "
